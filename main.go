...
func showCustomOverlay(overlay *fyne.Container, title string, content fyne.CanvasObject, btn1Text string, act1 func(), btn2Text string, act2 func()) {
	...
	// FIX: Tinggi Popup 350, Center Alignment (Presisi Tengah)
...
func main() {
	...
	// DEFINE UI ELEMENTS FIRST (TO AVOID "UNDEFINED" ERROR)
	...
	// DEFINE AUTOINSTALLKERNEL HERE (SO IT CAN SEE status & lblKernelValue)
	...
	// GOROUTINE MONITORING (SEKARANG AMAN KARENA LABEL SUDAH DIDEFINISIKAN)
	go func() {
		time.Sleep(1 * time.Second)
		for {
			func() {
	...
	// HELPER MODAL UPDATE (DEFINISI DI SINI AGAR BISA AKSES overlayContainer)
