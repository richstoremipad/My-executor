...
// --- CONFIG ---
// Ukuran font disesuaikan agar mirip terminal (10-11 pas untuk Android)
const TerminalFontSize = 11
...
type customBuffer struct {
	...
	lastBgColor color.Color // Warna Latar
	lastBold    bool
}
...
// --- CUSTOM RENDERER (Fix Background & Layout) ---
...
func (t *coloredText) Visual() fyne.CanvasObject {
	...
	// 3. Jika ada background, gunakan Stack (Tumpuk)
	// Rectangle di bawah, Text di atas.
	bg := canvas.NewRectangle(t.BgColor)
	
	// Stack akan menyesuaikan ukuran background dengan ukuran teks
	return container.NewStack(bg, text)
}

func (t *coloredText) Update(o fyne.CanvasObject) {
	// Update logic sederhana (biasanya Fyne membuat ulang objek untuk RichText)
}
...
func (cb *customBuffer) appendAnsiText(text string) {
	...
	// Ganti Tab dengan Spasi agar rapi (RichText Fyne kadang error dengan \t)
	text = strings.ReplaceAll(text, "\t", "    ")

	// Regex ANSI (Menangkap Warna FG, BG, dan Control Codes)
		...
		case "m": // --- WARNA ---
			...
			for _, c := range codes {
				...
				if val == 0 {
					...
					cb.lastBold = true
				} else if val >= 30 && val <= 37 { // Text Color
					cb.lastFgColor = ansiColors[val]
				} else if val >= 90 && val <= 97 { // Bright Text Color
					cb.lastFgColor = ansiColors[val]
				} else if val >= 40 && val <= 47 { // Background Color
					// Map 40-47 ke 30-37 untuk ambil warnanya dari map
...
func main() {
	...
	// [PENTING] Matikan Wrapping agar ASCII art 'XFILES' tidak hancur ke bawah
	outputRich.Wrapping = fyne.TextWrapOff 
	
	// ScrollBoth: Bisa geser kanan-kiri jika teks kepanjangan
	logScroll := container.NewScroll(outputRich)
	logScroll.Direction = container.ScrollBoth 

	// Background Hitam Pekat di belakang log
	...
	executeFile := func(reader fyne.URIReadCloser) {
		...
		go func() {
			...
			// ENV SETTINGS: Agar script tahu ukuran layar
