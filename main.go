...
func parseRawBytes(content []byte) ([]string, []string, []string, error) {
	...
	if len(ids) == 0 { return nil, nil, nil, errors.New("Data Kosong/Format Salah") }
...
func showGamePopup(w fyne.Window, overlay *fyne.Container, title string, content fyne.CanvasObject, 
    ...
    if content != nil { popupContainer.Add(container.NewPadded(content)) }
    
    buttonsContainer := container.NewHBox()
    if btn1Text != "" {
        btn1 := widget.NewButton(btn1Text, func() { overlay.Hide(); if act1 != nil { act1() } })
    ...
    if btn2Text != "" {
        ...
        btn2 := widget.NewButton(btn2Text, func() { overlay.Hide(); if act2 != nil { act2() } })
    ...
    wrapper.Resize(size)
    overlay.Objects = []fyne.CanvasObject{bg, container.NewCenter(wrapper)}
...
func showAccountListPopup(w fyne.Window, overlay *fyne.Container, term *Terminal, ids, names, displays []string, isOnline bool) {
    ...
    showGamePopup(w, overlay, "DAFTAR AKUN", listContainer,
        "BATAL", func() { if isOnline { removeFileRoot(OnlineAccFile) } },
        "PILIH", func() {
            if selectedIndex >= 0 {
...
func showDownloadErrorPopup(w fyne.Window, overlay *fyne.Container, term *Terminal) {
    content := container.NewVBox(
        ...
        widget.NewLabel(""), widget.NewLabel("Cek koneksi internet anda"),
    )
...
func showURLInputPopup(w fyne.Window, overlay *fyne.Container, term *Terminal) {
    ...
    content := container.NewVBox(widget.NewLabel("URL Akun Online:"), widget.NewSeparator(), urlEntry)
    
    showGamePopup(w, overlay, "INPUT URL", content, "BATAL", nil, "DOWNLOAD", func() {
            if urlEntry.Text != "" {
                // Simpan URL Config
                exec.Command("su", "-c", fmt.Sprintf("echo \"%s\" > %s", urlEntry.Text, UrlConfigFile)).Run()
                
                go func() {
                    term.Write([]byte("\x1b[33m[DL] Mengambil data (cURL Direct)...\x1b[0m\n"))
                    term.Write([]byte(fmt.Sprintf("\x1b[90m%s\x1b[0m\n", maskURL(urlEntry.Text))))
                    
                    // [OPTIMAL] cURL ke STDOUT (Tanpa simpan file dulu = Cepat)
                    cmd := exec.Command("su", "-c", fmt.Sprintf("curl -k -L -s --connect-timeout 10 \"%s\"", urlEntry.Text))
                    out, err := cmd.Output()
                    
                    if err == nil && len(out) > 10 {
                        term.Write([]byte("\x1b[32m[DL] Berhasil.\x1b[0m\n"))
                        
                        // Parse Langsung dari Memori
                        ...
                        if errParse == nil {
                            ...
                            // Simpan file backup di background (tidak memblokir UI)
                            go func() {
                                tmpFile := "/sdcard/accml_temp_save.ini"
                                os.WriteFile(tmpFile, out, 0666)
                                exec.Command("su", "-c", fmt.Sprintf("cp %s %s && rm %s", tmpFile, OnlineAccFile, tmpFile)).Run()
                            }()
                        } else {
                            term.Write([]byte("\x1b[31m[ERR] Format Salah.\x1b[0m\n"))
                            showDownloadErrorPopup(w, overlay, term)
                        }
                    } else {
                        term.Write([]byte("\x1b[31m[ERR] Gagal Download/Kosong.\x1b[0m\n"))
                        showDownloadErrorPopup(w, overlay, term)
                    }
                }()
            }
...
func showManualIDPopup(w fyne.Window, overlay *fyne.Container, term *Terminal) {
    entry := widget.NewEntry(); entry.SetPlaceHolder("Masukkan Device ID...")
    content := container.NewVBox(widget.NewLabel("Input Device ID Manual:"), widget.NewSeparator(), entry)
    showGamePopup(w, overlay, "INPUT MANUAL", content, "BATAL", nil, "TERAPKAN", func() {
            if len(entry.Text) > 5 {
                runMLBBTask(term, "Set Manual ID", func() {
                    term.Write([]byte(fmt.Sprintf("\x1b[36m[INFO] ID Baru: %s\x1b[0m\n", entry.Text)))
                    applyDeviceIDLogic(term, entry.Text, PackageNames[SelectedGameIdx], AppNames[SelectedGameIdx], "MANUAL")
                })
            }
        }, fyne.NewSize(350, 1200))
}
...
func showCustomOverlay(overlay *fyne.Container, title string, content fyne.CanvasObject, btn1Text string, act1 func(), btn2Text string, act2 func()) {
	...
	if btn2Text != "" {
		b1 := widget.NewButton(btn1Text, func(){ overlay.Hide(); if act1 != nil { act1() } }); b1.Importance = widget.DangerImportance
		b2 := widget.NewButton(btn2Text, func(){ overlay.Hide(); if act2 != nil { act2() } }); b2.Importance = widget.HighImportance
		...
		b1 := widget.NewButton(btn1Text, func(){ overlay.Hide(); if act1 != nil { act1() } }); b1.Importance = widget.HighImportance
		btnBox = container.NewHBox(layout.NewSpacer(), container.NewGridWrap(fyne.NewSize(110,40), b1), layout.NewSpacer())
	}
	card := widget.NewCard("", "", container.NewPadded(container.NewVBox(container.NewPadded(container.NewCenter(lblTitle)), container.NewPadded(content), widget.NewLabel(""), btnBox)))
...
func makeSideMenu(w fyne.Window, term *Terminal, overlayContainer *fyne.Container, onClose func()) (*fyne.Container, func()) {
	...
	selGame := widget.NewSelect(AppNames, func(s string) { for i, v := range AppNames { if v == s { SelectedGameIdx = i } } }); selGame.SetSelected(AppNames[0])
	...
	btnLogin := widget.NewButtonWithIcon("Login Akun", theme.LoginIcon(), func() {
		...
		btnBatal := widget.NewButton("BATAL", func() { overlayContainer.Hide() }); btnBatal.Importance = widget.DangerImportance
		content := container.NewVBox(container.NewGridWithColumns(2, btnOffline, btnOnline), layout.NewSpacer(), container.NewCenter(container.NewGridWrap(fyne.NewSize(120, 40), btnBatal)))

		btnOnline.OnTapped = func() {
			overlayContainer.Hide()
			var defaultUrl string
			if out, err := exec.Command("su", "-c", "cat "+UrlConfigFile).Output(); err == nil { defaultUrl = cleanString(string(out)) }
			
			if defaultUrl != "" && strings.HasPrefix(defaultUrl, "http") {
				// AUTO DOWNLOAD PAKE CURL MEMORY
				go func() {
					term.Write([]byte("\x1b[33m[DL] Auto-Download (cURL Direct)...\x1b[0m\n"))
					cmd := exec.Command("su", "-c", fmt.Sprintf("curl -k -L -s --connect-timeout 10 \"%s\"", defaultUrl))
					out, err := cmd.Output()
					if err == nil && len(out) > 10 {
						ids, names, displays, errParse := parseRawBytes(out)
						if errParse == nil {
							term.Write([]byte("\x1b[32m[OK] Sukses.\x1b[0m\n"))
							time.Sleep(100 * time.Millisecond)
							showAccountListPopup(w, overlayContainer, term, ids, names, displays, true)
						} else { showURLInputPopup(w, overlayContainer, term) }
					} else { showURLInputPopup(w, overlayContainer, term) }
				}()
			} else { showURLInputPopup(w, overlayContainer, term) }
		}
		...
		btnOffline.OnTapped = func() { 
			...
			if err == nil {
				term.Write([]byte(fmt.Sprintf("\x1b[32m[OK] %d Akun.\x1b[0m\n", len(ids))))
				showAccountListPopup(w, overlayContainer, term, ids, names, displays, false)
			} else { term.Write([]byte("\x1b[31m[ERR] File Gagal Dibaca.\x1b[0m\n")) }
		}
		showGamePopup(w, overlayContainer, "SUMBER AKUN", content, "", nil, "", nil, fyne.NewSize(350, 900))
	})
	...
	btnReset := widget.NewButtonWithIcon("Reset ID", theme.ViewRefreshIcon(), func() {
		...
		showGamePopup(w, overlayContainer, "RESET ID", content, "MANUAL", func() { showManualIDPopup(w, overlayContainer, term) }, "RANDOM", func() {
	...
	finalMenu := container.NewStack(container.NewStack(dimmer, btnDimmer), slideContainer); finalMenu.Hide()
...
func main() {
	...
	lblKernelTitle := createLabel("KERNEL", brightYellow, 10, true); lblKernelValue := createLabel("...", color.Gray{Y: 150}, 11, true)
	lblSELinuxTitle := createLabel("SELINUX", brightYellow, 10, true); lblSELinuxValue := createLabel("...", color.Gray{Y: 150}, 11, true)
	lblSystemTitle := createLabel("ROOT", brightYellow, 10, true); lblSystemValue := createLabel("...", color.Gray{Y: 150}, 11, true)
	...
	go func() {
		...
		for {
			func() {
				defer func() { if r := recover(); r != nil {} }()
				if CheckRoot() { lblSystemValue.Text = "GRANTED"; lblSystemValue.Color = successGreen } else { lblSystemValue.Text = "DENIED"; lblSystemValue.Color = failRed }; lblSystemValue.Refresh()
				if CheckKernelDriver() { lblKernelValue.Text = "ACTIVE"; lblKernelValue.Color = successGreen } else { lblKernelValue.Text = "MISSING"; lblKernelValue.Color = failRed }; lblKernelValue.Refresh()
	...
	executeTask := func(cmdText string, isScript bool, scriptPath string, isBinary bool) {
		...
		go func() {
			...
			if isScript {
				...
				if isRoot { cmd = exec.Command("su", "-c", fmt.Sprintf("cd \"%s\" && %s", currentDir, cmdText)) } else {
					runCmd := cmdText
					...
					if strings.HasPrefix(cmdText, "./") {
						...
						if strings.HasSuffix(fileName, ".sh") { runCmd = fmt.Sprintf("sh \"%s\"", fullPath) } else {
			...
			go func() { defer wg.Done(); io.Copy(term, stdout) }(); go func() { defer wg.Done(); io.Copy(term, stderr) }()
			wg.Wait(); cmd.Wait(); cmdMutex.Lock(); activeStdin = nil; cmdMutex.Unlock()
	...
	runFile := func(reader fyne.URIReadCloser) {
		...
		isBinary := bytes.HasPrefix(data, []byte("\x7fELF")); tmpFile, err := os.CreateTemp("", "exec_tmp")
		if err != nil { term.Write([]byte("\x1b[31m[ERR] Write Failed\x1b[0m\n")); return }
		tmpPath := tmpFile.Name(); tmpFile.Write(data); tmpFile.Close(); os.Chmod(tmpPath, 0755); executeTask("", true, tmpPath, isBinary)
	}
	...
	autoInstallKernel := func() {
		...
		go func() {
			...
			term.Write([]byte(fmt.Sprintf("Kernel: \x1b[33m%s\x1b[0m\n", fullVer))); targetKoPath := "/data/local/tmp/module_inject.ko"
			...
			term.Write([]byte(fmt.Sprintf("\x1b[32m[+] Menggunakan File: %s\x1b[0m\n", fileToExtract.Name))); rc, _ := fileToExtract.Open(); buf := new(bytes.Buffer); io.Copy(buf, rc); rc.Close()
	...
	btnInj := widget.NewButtonWithIcon("Inject", theme.DownloadIcon(), func() { showModal("INJECT", "Mulai Inject Driver?", "MULAI", func(){ autoInstallKernel() }, false, false) }); btnInj.Importance = widget.HighImportance
	...
	sendBtn := widget.NewButtonWithIcon("", theme.MailSendIcon(), send); cpyLbl := createLabel("Code by TANGSAN", silverColor, 10, false)
