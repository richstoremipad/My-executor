...
func makeSideMenu(w fyne.Window, term *Terminal, overlayContainer *fyne.Container, onClose func()) (*fyne.Container, func()) {
	...
	btnLogin := widget.NewButtonWithIcon("Login Akun", theme.LoginIcon(), func() {
		...
		showInputUrlFunc = func(msg string) {
			...
			labelMsg := widget.NewLabel(msg)
			labelMsg.Alignment = fyne.TextAlignCenter
	...
	// FIX: Layout Border agar Tombol Keluar di Bawah
...
func main() {
	...
	var overlayContainer *fyne.Container = container.NewStack()
	overlayContainer.Hide()
	...
	checkUpdate = func() {
		...
		if strings.Contains(ConfigURL, "GANTI") {
